...
tags: [goose, agy, multi-agent, terminal, benchmark, nebula]
---
...
- **Imperium captain** (fleet ship agent, same role as Claude Code on <Term id="imperator">Imperator</Term>): for general fleet work, <Term id="universalis">Universalis</Term> queries, engineering tasks
- **CHAOS** (<Term id="warp">Warp</Term> brain): tool-calling agent for the Warp pipeline — routes tasks to KHORNE/TZEENTCH/NURGLE neurons, reads/writes <Term id="anamnesis">Anamnesis</Term> DB
...
Goose's multi-call pattern is intentional: it separates tool-availability probing from actual inference. For a coding agent handling file edits and <Term id="shell">shell</Term> commands across a codebase, this is correct architecture. For a real-time fleet brain answering routing queries, it is structural overhead. The tool isn't wrong — it's deployed against the wrong job description.
</Admonition>
...
2. If model returns `tool_calls` → execute (psql for Anamnesis, <Term id="ssh">SSH</Term>+curl for <Term id="ollama">Ollama</Term> neurons, direct HTTP for Nurgle's <Term id="llama-server">llama-server</Term>)
