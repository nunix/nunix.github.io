...
The initial theory was <Term id="gtt">GTT</Term> (Graphics Translation Table) corruption — a known issue on AMD integrated graphics when VRAM pressure mounts. We ran tests, collected drm debug logs, checked for memory reclaim events. Nothing.
...
After the keyboard fix was deployed, the <Term id="emperor">Emperor</Term> ran the shortcut. The monitor woke. The keyboard continued working. The session held.
